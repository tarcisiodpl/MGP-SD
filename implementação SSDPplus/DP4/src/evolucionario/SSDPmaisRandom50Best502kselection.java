 ...
 */
public class SSDPmaisRandom50Best502kselection {
...
//        Avaliador.imprimirDimensaoQuantidade(P, P.length, 15);
        ...
        for(int numeroReinicializacoes = 0; numeroReinicializacoes < 3; numeroReinicializacoes++){//Controle número de reinicializações
            ...
            while(numeroGeracoesSemMelhoraPk < 3){
                ...
                PAsterisco = SELECAO.selecionarMelhores50Aleatorio50(P, Pnovo); 
                P = PAsterisco;   
    ...
    //            Avaliador.imprimirDimensaoQuantidade(Pk, k, 15);
        ...
        //SALVANDO MELHORES EM Pk: respeitando similaridade interna e diferenciação externa entre os centros.
        Pattern[] Pall = new Pattern[(witdh * Pattern.maxSimulares) + witdh];
        int indicePall = 0;
        for(int i = 0; i < Pwitdh.length; i++){
            Pattern p = Pwitdh[i];
            Pattern[] similares = p.getSimilares();
            if(similares != null){
                for(int j = 0; j < similares.length; j++){
                    Pall[indicePall++] = similares[j];                    
                }
            }else{
                for(int j = 0; j < Pattern.maxSimulares; j++){
                    Pall[indicePall++] = new Pattern(new HashSet<Integer>(), tipoAvaliacao);
                }
            }
            Pall[indicePall++] = new Pattern(Pwitdh[i].getItens(), Pwitdh[i].getTipoAvaliacao());
        }
        Arrays.sort(Pall);
        
        //Preenchendo Pk com DPs vazias.
        Pattern[] Pk = new Pattern[k];
        for(int i = 0; i < Pk.length; i++){
            Pk[i] = new Pattern(new HashSet<Integer>(), tipoAvaliacao);
        }
        
        //Preecher Pk respeitando similaridade interna e externa.
        SELECAO.salvandoRelevantesDPmais(Pk, Pall, similaridade);
